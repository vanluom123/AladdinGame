...
CivilianEnemy1st::CivilianEnemy1st(GVector2 position, Player* mplayer)
{
	this->Tag = Entity::CivilianEnemy1st;
	this->HP = 1;
	this->mCivilianEnemy1stData = new CivilianEnemy1stData();
...
void CivilianEnemy1st::Draw(GVector3 position, RECT sourceRect, GVector2 scale, GVector2 transform, float angle, GVector2 rotationCenter, D3DXCOLOR colorKey)
{
	if (IsNearToPlayer() == true)
	{
...
void CivilianEnemy1st::Draw(GVector2 transform)
{
	if (IsNearToPlayer() == true)
	{
		mCurrentAnimation->SetPosition(this->GetPosition());
...
void CivilianEnemy1st::OnSetPosition(GVector3 pos)
{
