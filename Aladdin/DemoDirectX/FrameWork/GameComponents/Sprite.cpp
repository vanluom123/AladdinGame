...
{
	GVector3 pPosition = mPosition;
	RECT pSrcRect = mSourceRect;
	float rotation = mRotation;
	GVector2 pScaling = mScale;
	GVector2 pTranslation = mTranslation;
	GVector2 pRotationCenter = mRotationCenter;
	GVector2 pScalingCenter(pPosition.x, pPosition.y);
	...
	if (rotationCenter != GVector2())
		...
		mRotationCenter = GVector2(pPosition.x, pPosition.y);// cho phep quay giua hinh
